...
func TestGetOrFile_ReadsEnvVars(t *testing.T) {
	err := os.Setenv("TEST_LEGO_ENV_VAR", "cert-manager_env")
	require.NoError(t, err)
	defer os.Unsetenv("TEST_LEGO_ENV_VAR")

	value := GetOrFile("TEST_LEGO_ENV_VAR")

	assert.Equal(t, "cert-manager_env", value)
}
...
func TestGetOrFile_ReadsFiles(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			desc:        "simple",
			fileContent: []byte("cert-manager_file"),
		},
		{
	...
	for _, test := range testCases {
		t.Run(test.desc, func(t *testing.T) {
			...
			file, err := ioutil.TempFile("", "cert-manager")
			require.NoError(t, err)
			defer os.Remove(file.Name())

			err = ioutil.WriteFile(file.Name(), []byte("cert-manager_file\n"), 0644)
			require.NoError(t, err)
			...
			assert.Equal(t, "cert-manager_file", value)
		})
	}
}
...
func TestGetOrFile_PrefersEnvVars(t *testing.T) {
	...
	err = ioutil.WriteFile(file.Name(), []byte("cert-manager_file"), 0644)
	require.NoError(t, err)
	...
	err = os.Setenv(varEnvName, "cert-manager_env")
	require.NoError(t, err)
