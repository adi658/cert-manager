...
2. Implement functions to match the solver interface (``Present``, ``CleanUp`` and ``Timeout``).
   ...
   https://github.com/xenolf/cert-manager, so if cert-manager supports the DNS provider you
   ...
   - replace uses of ``github.com/xenolf/cert-manager/acme`` with ``github.com/adi658/cert-manager/pkg/issuer/acme/dns/util``.
   - replace uses of ``github.com/xenolf/cert-manager/log`` with ``github.com/golang/glog``.
   - remove references to ``github.com/xenolf/cert-manager/platform/config/env``.
