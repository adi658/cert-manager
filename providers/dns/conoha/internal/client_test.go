...
func TestClient_CreateRecord(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			handler: func(rw http.ResponseWriter, req *http.Request) {
				...
				if string(raw) != `{"name":"cert-manager.com.","type":"TXT","data":"txtTXTtxt","ttl":300}` {
	...
	for _, test := range testCases {
		t.Run(test.desc, func(t *testing.T) {
			...
			mux.Handle("/v1/domains/cert-manager/records", test.handler)

			domainID := "cert-manager"

			record := Record{
				Name: "cert-manager.com.",
				Type: "TXT",
				Data: "txtTXTtxt",
				TTL:  300,
			}
