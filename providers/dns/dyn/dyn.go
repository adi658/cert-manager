...
import (
	...
	"github.com/adi658/cert-manager/platform/config/env"
)
...
func (d *DNSProvider) Present(domain, token, keyAuth string) error {
	...
	err = d.publish(authZone, "Added TXT record for ACME dns-01 challenge using cert-manager client")
	if err != nil {
		return fmt.Errorf("dyn: %v", err)
	}
...
func (d *DNSProvider) CleanUp(domain, token, keyAuth string) error {
	...
	err = d.publish(authZone, "Removed TXT record for ACME dns-01 challenge using cert-manager client")
	if err != nil {
		return fmt.Errorf("dyn: %v", err)
	}
