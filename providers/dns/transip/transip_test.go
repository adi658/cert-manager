...
func TestDNSProvider_concurrentGetInfo(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		err := solve("bar.cert-manager.wtf", ".bar", 500*time.Millisecond, 100*time.Millisecond, 100*time.Millisecond)
		require.NoError(t, err)
	}()

	go func() {
		defer wg.Done()
		err := solve("foo.cert-manager.wtf", ".foo", 500*time.Millisecond, 200*time.Millisecond, 100*time.Millisecond)
		require.NoError(t, err)
	}()
...
func TestDNSProvider_concurrentSetDNSEntries(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		err := solve("bar.cert-manager.wtf", 550*time.Millisecond, 500*time.Millisecond)
		require.NoError(t, err)
	}()

	go func() {
		defer wg.Done()
		err := solve("foo.cert-manager.wtf", 500*time.Millisecond, 100*time.Millisecond)
		require.NoError(t, err)
	}()
