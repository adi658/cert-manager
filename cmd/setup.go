...
func setup(ctx *cli.Context, accountsStorage *AccountsStorage) (*Account, *cert-manager.Client) {
...
func newClient(ctx *cli.Context, acc registration.User, keyType certcrypto.KeyType) *cert-manager.Client {
	config := cert-manager.NewConfig(acc)
	config.CADirURL = ctx.GlobalString("server")

	config.Certificate = cert-manager.CertificateConfig{
		KeyType: keyType,
		Timeout: time.Duration(ctx.GlobalInt("cert.timeout")) * time.Second,
	}
	config.UserAgent = fmt.Sprintf("cert-manager-cli/%s", ctx.App.Version)

	if ctx.GlobalIsSet("http-timeout") {
		config.HTTPClient.Timeout = time.Duration(ctx.GlobalInt("http-timeout")) * time.Second
	}

	client, err := cert-manager.NewClient(config)
	if err != nil {
