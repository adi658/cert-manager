...
import (
	...
	"github.com/adi658/certmanager/certcrypto"
	"github.com/adi658/certmanager/lego"
	"github.com/adi658/certmanager/log"
	"github.com/adi658/certmanager/registration"
	"github.com/urfave/cli"
)

const filePerm os.FileMode = 0600

func setup(ctx *cli.Context, accountsStorage *AccountsStorage) (*Account, *certmanager.Client) {
...
func newClient(ctx *cli.Context, acc registration.User, keyType certcrypto.KeyType) *certmanager.Client {
	config := certmanager.NewConfig(acc)
	config.CADirURL = ctx.GlobalString("server")

	config.Certificate = certmanager.CertificateConfig{
		KeyType: keyType,
		Timeout: time.Duration(ctx.GlobalInt("cert.timeout")) * time.Second,
	}
	config.UserAgent = fmt.Sprintf("certmanager-cli/%s", ctx.App.Version)

	if ctx.GlobalIsSet("http-timeout") {
		config.HTTPClient.Timeout = time.Duration(ctx.GlobalInt("http-timeout")) * time.Second
	}

	client, err := certmanager.NewClient(config)
	if err != nil {
